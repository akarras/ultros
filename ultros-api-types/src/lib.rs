...
mod tests {
    ...
    fn test_sale(id: i32, item_id: i32, price: i32) -> SaleHistory {
        SaleHistory {
            id,
            quantity: 1,
    ...
    fn test_apply_sales_event_add() {
        ...
        let event = EventType::Added(SaleEventData {
            sales: vec![(test_sale(1, 1, 100), crate::UnknownCharacter { id: 1, name: "Buyer".into() })],
        });
    ...
    fn test_apply_sales_event_wrong_item_id() {
        let mut data = CurrentlyShownItem {
            listings: vec![],
            sales: vec![test_sale(1, 1, 100)],
        };
        // Event for item 2
        let event = EventType::Added(SaleEventData {
            sales: vec![(test_sale(2, 2, 200), crate::UnknownCharacter { id: 1, name: "Buyer".into() })],
        });

        data.apply_sales_event(1, event);
        // Should still only have the sale for item 1
        assert_eq!(data.sales.len(), 1);
        assert_eq!(data.sales[0].id, 1);
    }
