...
pub const BOT_REMOVED_REASON: &str = "The Ultros bot is no longer in the linked Discord server, so this group's membership \
...
pub async fn on_member_removal(db: &UltrosDb, guild_id: i64, user_id: i64) -> Result<SyncSummary> {
    let roles = db.synced_roles_for_guild(guild_id).await?;
    if roles.is_empty() {
        return Ok(SyncSummary::default());
    }
...
pub async fn on_role_delete(db: &UltrosDb, guild_id: i64, discord_role_id: i64) -> Result<()> {
...
/// bot was away.
pub fn on_guild_create(guild_id: i64) {
