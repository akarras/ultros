...
## 2026-09-09 - [Fix IDOR in Discord DM Endpoint Creation]
**Vulnerability:** Insecure Direct Object Reference (IDOR)
**Learning:** `EndpointMethod::DiscordDm { user_id }` was accepted straight from the request body, so any authenticated user could create (or update) an alert endpoint that delivered DMs to an arbitrary Discord account. `create_endpoint` already rewrote the frontend's `user_id: 0` sentinel to the caller's id, but `update_endpoint` did not, and neither path ever compared a non-zero `user_id` against the owner.
**Prevention:** Resolve the `user_id: 0` sentinel on every write path, then pass the authenticated `owner_id` into `validate_endpoint_method` and reject any payload whose `user_id` differs. Ownership fields inside a serialized config blob need the same authorization check as a top-level path parameter.
