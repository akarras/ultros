...
## 2024-06-25 - Avoid heap allocations when parsing substrings
**Learning:** We can write a zero-allocation string search algorithm that performs identical substring matching without the need to allocate intermediate  instances with  or  in hot parsing paths.
**Action:** When working on parsing logic (e.g. FFXIV tags parsing), prefer manual string searches using `eq_ignore_ascii_case` over creating `String` using `.to_lowercase()` for simple matching tasks, as this avoids unnecessary heap allocations and garbage collection overhead.
## 2026-06-25 - Avoid heap allocations when parsing substrings (update)
**Learning:** We can write a zero-allocation string search algorithm that performs identical substring matching without the need to allocate intermediate `String` instances with `.to_lowercase()` in hot parsing paths like render loops.
**Action:** When working on parsing logic, prefer manual string searches using `eq_ignore_ascii_case` over creating `String` using `.to_lowercase()` for simple matching tasks, as this avoids unnecessary heap allocations and garbage collection overhead.
