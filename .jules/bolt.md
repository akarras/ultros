...
## 2026-11-20 - Avoid unnecessary clones and allocation in filter/retention loops
**Learning:** In `ultros-frontend/ultros-app/src/analysis.rs`, the `sniper_clamp` function was making an unnecessary full `.clone()` of a `Vec<i32>` just to calculate a median, and then iterating over it to `.collect()` into a new vector. We can sort the array in place to find the median, and then use `retain()` to filter elements in-place instead of creating a new `Vec`. In `ultros-frontend/ultros-app/src/routes/analyzer.rs`, a similar `.clone()` of `clamped` was passed to `filter_outliers_iqr_in_place`, which was unneeded as we no longer need the original order for the remainder of the function.
**Action:** When filtering a vector based on calculations on the vector itself (like median), do it in-place using `.retain()` instead of `.clone()` and `.collect()` to avoid expensive heap allocations.
