...
## 2024-05-15 - Use linear time select_nth_unstable_by_key instead of O(N log N) full sort in live_sale_ticker.rs
...
## 2026-08-16 - Avoid unnecessary clones and allocation in filter/retention loops
**Learning:** In `ultros-frontend/ultros-app/src/analysis.rs`, the `sniper_clamp` function was making an unnecessary full `.clone()` of a `Vec<i32>` just to calculate a median, and then iterating over it to `.collect()` into a new vector. Reordering the vector in place (`median_in_place_i32` partitions via `select_nth_unstable` — it does not fully sort) gives the same median, and `retain()` then filters in place instead of allocating a new `Vec`. In `ultros-frontend/ultros-app/src/routes/analyzer.rs`, a similar `.clone()` of `clamped` was passed to `filter_outliers_iqr_in_place`; that one was simply a move the borrow checker already allowed, since `clamped` is dead afterwards.
**Action:** When filtering a vector based on a calculation over the vector itself (like a median), do it in place with `.retain()` instead of `.clone()` + `.collect()`. **Cost:** the result's order becomes undefined — check every caller is order-independent and document the new contract, the way `filter_outliers_iqr_in_place` does.
...
## 2026-08-30 - Batch O(N) searches inside Leptos For components into a single Memo
