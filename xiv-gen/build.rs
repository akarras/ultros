...
fn create_struct(
    ...
    let mut detectors: Vec<DataDetector> = headers
        .iter()
        .enumerate()
    ...
    let fields: Vec<(String, String, String)> = headers
        .iter()
        ...
        .map(|(field_name, sample_data)| {
            ...
            let rust_type = sample_data.field_type(csv_name);
            ...
            if line_one == "key_id" {
                ...
                apply_derives(&mut key)
                    ...
                    .derive("Copy")
                    .vis("pub")
                    .tuple_field(&rust_type)
                    .vis("pub");
                scope.push_struct(key);

                let db_field_name = format!("{}s", csv_name.to_snake_case());
                let key_value = match sample_data {
                    DataType::ReferenceKey => Cow::from(format!("Vec<{csv_name}>")),
                    _ => Cow::Borrowed(csv_name.as_str()),
                };
                ...
                return (line_one, key_name.clone(), field_name.clone());
            }

            (line_one, rust_type, field_name.clone())
        })
        .collect();
    if fields.len() > 100 {
        ...
        for (key, value, _header) in &fields {
            lazy_static! {
                // regex: check is number
            ...
            if let Some(captures) = DOUBLE.captures(key) {
                ...
                {
                    if let KeyType::Single(c) = k {
                        *c += 1;
                    }
                } else {
                    root_names.push((root, (KeyType::Single(1), value), 0));
                }
                ...
                if let Some((_, (k, _), _)) = root_names.iter_mut().find(|(key, _, _)| key == root)
                {
                    ...
                    root_names.push((root.to_string(), (KeyType::Single(1), value), 0));
                }
            } else {
        ...
        for (field_name, field_value, header_name) in fields.iter() {
            let mut field = Field::new(field_name, field_value).vis("pub").to_owned();
            let mut annotations = vec![format!("#[serde(rename = \"{}\")]", header_name)];
            if field_value == "i64" {
                annotations.push(
                    "#[serde(deserialize_with = \"deserialize_i64_from_u8_array\")]".to_string(),
                );
            }
            if field_value == "bool" {
                annotations.push(
            ...
            if field_value.ends_with("Id") {
                annotations.push(r#"#[serde(deserialize_with = "ok_or_default")]"#.to_string());
            }
            field.annotation(annotations.iter().map(|s| s.as_str()).collect());
            s.push_field(field);
        }
    }
