                    ...
                    .map(|shop| {
                        ...
                        view! {
                            <div class="group flex flex-col gap-2 rounded-lg card p-3 transition-colors h-full">
                                <div class="text-sm font-medium border-b border-[color:var(--color-outline)] pb-2 mb-1 text-[color:var(--color-text)]">
                                    {shop.name.as_str()}
                                </div>
                                <div class="flex flex-col gap-1.5 flex-1">
                                    <span class="text-xs text-[color:var(--color-text-muted)] font-semibold uppercase tracking-wide">"Costs"</span>
                                    <div class="flex flex-col gap-1.5">
                                        {all_costs.map(|(item_id, count)| {
                                            if let Some(item) = data.items.get(item_id) {
                                                view! {
                                                    <div class="flex items-center gap-2 bg-[color:var(--color-base)]/50 px-2 py-1 rounded border border-[color:var(--color-outline)]">
                                                        <span class="text-sm font-bold min-w-[1.5rem] text-right">{*count}</span>
                                                        <span class="text-xs text-[color:var(--color-text-muted)]">"x"</span>
