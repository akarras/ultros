use crate::{
    api::get_retainer_listings,
    components::{gil::*, item_icon::*, retainer_nav::*, world_name::*},
};
use leptos::*;
...
#[component]
fn UndercutCharacterRetainerList(
    cx: Scope,
                        ...
                        match retainer {
                            Some(retainers) => {
                                let retainers : Vec<_> = retainers.retainers.into_iter()
                                    .map(|(character, retainers)| view!{cx, <UndercutCharacterRetainerList character retainers />})
                                    .collect();
