...
pub fn CategoryItems() -> impl IntoView {
    ...
    let canonical_href = move || {
        let params = params();
        let cat_param = params.get_str("category").unwrap_or("");
        if let Some(category) = resolve_category_param(data, cat_param) {
            format!("https://ultros.app/items/category/{}", category.key_id.0)
        } else {
            format!("https://ultros.app/items/category/{cat_param}")
        }
    };
    view! {
...
pub fn JobItems() -> impl IntoView {
    ...
    // Note: The canonical URL strips `?show-non-market=`, which genuinely changes
    // the item set (by including/excluding unmarketable items). We treat the
    // default (market board listable items only) as the canonical representation
    // of the jobset page to minimize crawling thin content.
    let canonical_href = move || {
        let params = params();
        let jobset = params.get_str("jobset").unwrap_or("");
        let encoded_jobset =
            percent_encoding::utf8_percent_encode(jobset, PATH_SEGMENT).to_string();
        format!("https://ultros.app/items/jobset/{encoded_jobset}")
    };
