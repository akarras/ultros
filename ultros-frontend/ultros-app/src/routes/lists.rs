        ...
        <div class="flex flex-col gap-4">
            ...
            <Suspense fallback=move || view! { <BoxSkeleton rows=1 /> }>
                {move || match user_resource.get() {
                    ...
                    Some(None) => {
                        if device_lists.get() {
                            return view! { <p class="text-sm opacity-70">"Have lists on another device? "<a class="underline" rel="external" href="/login?next=/list?labs=lists-sync">"Sign in to your account"</a></p> }.into_any();
                        }
                        view! {
