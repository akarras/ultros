...
pub fn permission_label(permission: ListPermission) -> &'static str {
    match permission {
...
pub fn editable_permission(value: &str) -> ListPermission {
    match value {
...
pub fn invite_url(invite_id: &str) -> String {
    invite_link::invite_url(LIST_INVITE_PATH, invite_id)
}

pub fn copy_invite_url(
    invite_id: &str,
...
pub fn invite_uses_label(invite: &ListInvite) -> String {
...
#[component]
pub fn ShareListSection(
    list: List,
    #[prop(into, optional)] refresh_signal: Option<Signal<u32>>,
) -> impl IntoView {
    let i18n = use_i18n();
    let list_id = list.id;
    let method = RwSignal::new("link");
    ...
    <section class="space-y-3" class:hidden=move || method.get() != "user">
...
#[component]
pub fn ShareListModal(list: List, set_visible: WriteSignal<bool>) -> impl IntoView {
    let i18n = use_i18n();
