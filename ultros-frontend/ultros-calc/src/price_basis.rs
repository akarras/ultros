//! Legacy map-cloning oracles and compatibility-enum regression tests.
//! These pin the behavior of the lazy price views in [`crate::pricing`].

pub use crate::formula::{BuyScope, CostBasis, RevenueMetric};

// The two map-cloning helpers below are the oracle `SignalView` is tested
// against; nothing outside tests calls them any more.
#[cfg(test)]
use crate::formula::SaleStat;
#[cfg(test)]
