...
pub(crate) async fn register(ctx: Context<'_>,
    #[description = "name of your ffxiv character"] name: String,
    #[description = "world your character is on"] home_world: Option<String>) -> Result<(), Error> {
                    ...
                    row.create_select_menu(|select| {
                        select.custom_id("RegisterCharacterSelect").options(|o| {
                            for search_result in profiles {
                                o.create_option(|o| {
                                    o.description("test").label(format!("{}\n{}", search_result.name, search_result.world)).value(search_result.user_id)
                                });
                            }
                            o
                        })
        ...
        let selected_value = if let Some(msg) = msg.message().await?.await_component_interaction(ctx.discord())
            .timeout(Duration::from_secs(5 * 60)).await {
                // msg.data.values[0]
