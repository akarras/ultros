...
fn build_item_search_schema() -> Result<Index> {
  ...
  for (id, item) in items {
    let category = categories.get(&item.item_ui_category).map(|m| m.name.to_string()).unwrap_or_default();
...
pub fn do_query(query_str: &str) -> Result<Vec<(i32, &'static Item)>> {
  ...
  let name = ITEM_INDEX.schema().get_field("name").ok_or(anyhow::Error::msg("Unable to get name field"))?;
  let category = ITEM_INDEX.schema().get_field("category").ok_or(anyhow::Error::msg("Unable to get category field"))?;
  ...
  results.iter().map(|(_, item)| {
    let doc = searcher.doc(*item)?;
    let item_id = doc.field_values()[0].value.as_i64().ok_or(anyhow::Error::msg("Unexpected field data type"))? as i32;
    let item = xiv_gen::ItemId(item_id);
    let item = items.get(&item).ok_or(anyhow::Error::msg("Item not found"))?;
    Ok((item_id, item))
  }).collect()
}
...
#[cfg(test)]
mod test {
    use tantivy::{query::QueryParser, collector::TopDocs};
