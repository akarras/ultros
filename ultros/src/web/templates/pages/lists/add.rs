...
pub(crate) async fn add_list(
    ...
    if let Some(Query(AddListQueryItems { list, world })) = &query {
        let user = db
            .get_or_create_discord_user(auth_user.id, auth_user.name.clone())
            .await?;
        db.create_list(
            user,
            list.clone(),
            ultros_db::world_cache::AnySelector::World(*world),
        )
        .await?;
    }
    Ok(RenderPage(AddListPage {
        user: auth_user,
        world_cache,
        success: query.map(|Query(q)| q.list),
    }))
}
