...
pub(crate) async fn refresh_character(State(db): State<UltrosDb>, Path(character_id): Path<i32>) -> Result<Redirect, WebError> {
    let mut character = db.get_character(character_id).await?.ok_or(anyhow::Error::msg("Character not in database"))?;
