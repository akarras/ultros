...
pub(crate) async fn increase_weight_retainer(Path(owned_retainer_id): Path<i32>, State(db): State<UltrosDb>, user: AuthDiscordUser) -> Result<Redirect, WebError> {
    db
        .update_owned_retainer(user.id as i64, owned_retainer_id, move |mut u| {
...
pub(crate) async fn decrease_weight_retainer(Path(owned_retainer_id): Path<i32>, State(db): State<UltrosDb>, user: AuthDiscordUser) -> Result<Redirect, WebError> {
    db
        .update_owned_retainer(user.id as i64, owned_retainer_id, move |mut u| {
            let current_weight = u.weight.take().unwrap_or_default().unwrap_or_default();
            u.weight = ActiveValue::Set(Some(current_weight - 1));
