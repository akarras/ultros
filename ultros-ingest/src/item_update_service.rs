...
#[derive(Default)]
pub struct SweepLock(AtomicBool);
...
impl SweepLock {
    pub fn try_claim(self: &Arc<Self>) -> Option<SweepLockGuard> {
        self.0
...
impl SweepLockGuard {
    ...
    pub async fn run<F: std::future::Future>(&mut self, work: F) -> Option<F::Output> {
...
pub struct UpdateService {
    ...
    pub sales: EventProducer<SaleEventData>,
    /// ClickHouse `listing_events` mirror for every board this service writes.
    pub listing_events: ultros_clickhouse::writer::Writer<ultros_clickhouse::rows::ListingEventRow>,
...
impl UpdateService {
    pub fn start_service(service: Arc<Self>, token: CancellationToken) {
    ...
    pub fn all_marketable_items() -> Box<[i32]> {
    ...
    pub async fn try_begin_full_sweep(&self) -> Option<SweepLockGuard> {
        #[cfg(feature = "test-auth")]
        if crate::test_market_isolation::enabled() {
            return None;
        }
    ...
    /// to resume.
    pub async fn resume_sweep(&self) -> Result<Option<SweepRun>, SeaDbErr> {
    ...
    pub async fn begin_or_resume_sweep(
        &self,
    ...
    pub async fn do_full_world_sweep(
        &self,
        run: SweepRun,
