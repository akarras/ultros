...
3b. **The formula-role marks come with the sub-label.** The recipe's headers carry `TermBadge` (`components/term_badge.rs`: `=` result, `+` revenue, `−` cost / tax, fill-weight not hue, sr-only role name) through `AnalyzerGrid`'s `ToolColumnMeta.side` + `MarkLabels`; the six `MarketGrid` consumers render bare labels in their own `header=` closures and none uses the badge (decided 2026-09-08, Aaron's ask). `MarketGrid` gains a `roles` prop mapping column id → `(TermRole, sub-label)`, rendered by the shared two-line header cell (`grid.rs` `header_cell` / `HeaderLine2`, `heading_adornments = 26.0` so auto-fit reserves the chip). The mark follows the basis: when `?revenue=sale-median` makes `market-sale-median` the revenue term, the `+` and the "(= Price)"-style hint move to that column exactly as the recipe's `pill_param` does. Per page: Flip Finder `profit` = / `sale_estimate` + / `buy_price` −; Vendor Resale `profit` / `market-price` / `vendor-price`; FC Crafting `profit` / `market-price` / `cost`; Leve `profit` / `revenue` / `cost`; Venture `profit` / `unit-price` (no cost column: the venture cost is a fixed term); Scrip Sources `cost-per-scrip` = / `cost` − (no revenue: unmarketable outputs); Currency Exchange (WS-G) `profit` / `price_per_item` / `cost`. Trends, Item Explorer and Retainer listings have no ledger and get no marks.
3c. **The "use" pill comes too** (Aaron, 2026-09-08). On the recipe, a signal column's header carries a `HeaderPill` rendered as `SortableHeaderCell`'s `trailing` slot (`grid.rs:224-235`); clicking it writes exactly one URL param through the filter signal — `?revenue=` or `?cost-basis=`, side chosen by the column kind (`pill_param`, `recipe_analyzer.rs:1753`), default stripped — and the header of the column that *is* the current term reads "(= Price)" / "(= Cost / unit)" instead. The matrix from #1313 was built without this: `market-sale-{min,median,avg}` and `market-listing` are exactly the four `PriceSignal`s the basis picker offers, yet the only way to pick one is the `MarketPriceControls` select. WS-A gives every shared column whose statistic is a `PriceSignal` the same pill: "use" writes the page's basis param (`?revenue=` on revenue-side subjects, `?cost-basis=` on cost-side ones — the side comes from the same `roles` prop as 3b, since a `MarketSubject` sits on exactly one side of its page's ledger), the current term's header shows "(= <term>)" in place of the pill, and `MarketPriceControls` stays as the picker for people who prefer a select. **Pinned windows:** the page has one window, so "use" on a pinned column such as `market-sale-median-30` writes `revenue=sale-median` *and* `window=30` (every follow column moves with it, which is the model); a window-pinned basis token (`sale-median-30`) is the escape hatch if a mixed view turns out to be wanted, and is deliberately not built now. Recipe ids keep their own pill wiring unchanged.
