...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.create_index(IndexCreateStatement::new().table(SaleHistory::Table)
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.drop_index(IndexDropStatement::new().table(SaleHistory::Table).name("SaleHistoryFullIndex").to_owned()).await?;
        Ok(())
    }
}