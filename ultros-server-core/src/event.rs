...
pub type EventBus<T> = tokio::sync::broadcast::Receiver<EventType<Arc<T>>>;
pub type EventProducer<T> = tokio::sync::broadcast::Sender<EventType<Arc<T>>>;
...
#[derive(Debug)]
pub struct ListDocEvent {
    pub list_id: i32,
    pub update: Vec<u8>,
    pub origin_socket: Option<u64>,
}
...
impl<T> From<EventType<T>> for ultros_api_types::websocket::EventType<T> {
    fn from(value: EventType<T>) -> Self {
        use ultros_api_types::websocket::EventType as WEvent;
        match value {
...
pub fn create_event_busses() -> (EventSenders, EventReceivers) {
...
pub struct EventSenders {
    pub retainers: EventProducer<OwnedRetainer>,
    pub listings: EventProducer<ListingEventData>,
    pub alerts: EventProducer<alert::Model>,
    pub retainer_undercut: EventProducer<alert_retainer_undercut::Model>,
    pub history: EventProducer<SaleEventData>,
    pub lists: EventProducer<ListEventData>,
    /// Sent by `crate::lists::ListSync::publish`.
    pub list_docs: EventProducer<ListDocEvent>,
...
pub struct EventReceivers {
    pub retainers: EventBus<OwnedRetainer>,
    pub listings: EventBus<ListingEventData>,
    pub alerts: EventBus<alert::Model>,
    pub retainer_undercut: EventBus<alert_retainer_undercut::Model>,
    pub history: EventBus<SaleEventData>,
    pub lists: EventBus<ListEventData>,
    pub list_docs: EventBus<ListDocEvent>,
    pub notifications: EventBus<NotificationEvent>,
}
...
/// iteration.
pub fn handle_bus_recv<T>(bus: &'static str, result: Result<T, RecvError>) -> BusRecv<T> {
    match result {
