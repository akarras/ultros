...
pub fn get_serenity_ctx() -> Option<Arc<serenity_prelude::Context>> {
...
pub fn get_web_push_config() -> Option<&'static WebPushConfig> {
    WEB_PUSH_CONFIG.get()
}
...
pub fn parse_endpoint_config(method: &str, config: &serde_json::Value) -> Result<EndpointConfig> {
    let mut config_obj =
...
impl PushOptions {
    ...
    pub fn for_alert(kind: AlertKind, alert_id: i32, click_url: impl Into<String>) -> Self {
        Self {
    ...
    pub fn immediate(click_url: impl Into<String>) -> Self {
        Self {
            click_url: click_url.into(),
            ttl_secs: 5 * 60,
            topic: None,
...
pub async fn deliver_non_discord_endpoint(
    endpoint: &ultros_db::entity::notification_endpoint::Model,
    title: &str,
    body: &str,
    push: &PushOptions,
...
pub fn permanent_failure_reason(err: &anyhow::Error) -> Option<String> {
...
pub async fn dispatch_alert_detailed(
    alert_id: i32,
    title: &str,
    body: &str,
    push: &PushOptions,
...
pub fn validate_web_push_endpoint(endpoint: &str) -> Result<url::Url, &'static str> {
