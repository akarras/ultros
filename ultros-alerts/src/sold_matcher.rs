...
pub const MAX_SALE_AGE_SECONDS: i64 = 24 * 3600;
...
impl SoldMatcher {
    pub fn new(owned: HashSet<i32>) -> Self {
        Self {
            owned,
    ...
    pub fn set_owned(&mut self, owned: HashSet<i32>) {
        self.owned = owned;
    }

    pub fn on_removed(&mut self, listing: RemovedListing, now: DateTime<Utc>) {
    ...
    pub fn on_added(&mut self, added: AddedListing) {
    ...
    pub fn on_sale(&mut self, sale: ObservedSale, now: DateTime<Utc>) {
    ...
    pub fn settle(&mut self, now: DateTime<Utc>) -> Vec<SoldEvent> {
        self.expire(now);
