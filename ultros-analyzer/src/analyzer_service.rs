...
pub struct SaleHistory {
    pub item_map: BTreeMap<ItemKey, arrayvec::ArrayVec<SaleSummary, SALE_HISTORY_SIZE>>,
}

impl SaleHistory {
    pub fn add_sale<'a, T>(&mut self, sale: &'a T)
    where
...
#[archive(check_bytes)]
pub struct CheapestListingValue {
    pub price: i32,
    pub world_id: i32,
}
...
pub struct CheapestListings {
    pub item_map: BTreeMap<ItemKey, CheapestListingValue>,
}
...
impl AnalyzerService {
    ...
    pub async fn get_trends(&self, world_id: i32) -> Option<TrendsData> {
        if !self.initiated.load(Ordering::Relaxed) {
            return None;
        }
    ...
    pub async fn read_cheapest_items<T, O>(
        &self,
        selector: &AnySelector,
        extract: T,
    ...
    pub async fn read_sale_history<T, O>(
        &self,
        selector: &AnySelector,
        extract: T,
    ) -> Result<O, AnalyzerError>
...
impl<'a> FromIterator<&'a SaleSummary> for SoldWithin {
    fn from_iter<T: IntoIterator<Item = &'a SaleSummary>>(iter: T) -> Self {
        SoldWithin::calculate(iter, Utc::now().naive_utc())
    }
}
...
pub struct SaleHistoryStats {
    /// Lower-middle median of the in-window prices.
    pub median: i32,
    pub sold_within: SoldWithin,
...
#[derive(Default)]
pub struct ResaleOptions {
    pub minimum_profit: Option<i32>,
    pub filter_world: Option<i32>,
    pub filter_datacenter: Option<i32>,
    pub filter_sale: Option<SoldWithin>,
    ...
    pub min_velocity_per_day: Option<f32>,
